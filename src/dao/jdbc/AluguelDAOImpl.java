...
public class AluguelDAOImpl implements AluguelDAO {
	...
	public void edit(Connection conn, Aluguel aluguel) throws Exception {
		...
		myStmt.executeUpdate();
		conn.commit();
		System.out.println("Aluguel atualizado efetuado !");
	...
	public Collection<Aluguel> list(Connection conn) throws Exception {
		...
		Collection<Aluguel> aluguel = new ArrayList<>();
        ...
        while (rs.next()) {
            ...
            aluguel.add(new Aluguel(idAluguel,(List<Filme>) filme.list(conn),new ClienteDAOImpl().find(conn, rs.getInt("id_cliente")), data, valor));
        }
		return aluguel;
	}
